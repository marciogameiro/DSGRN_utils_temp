# SaveDatabaseJSON_DSGRN_Plus_CC.py
# Marcio Gameiro
# MIT LICENSE
# 2021-03-06
...
def blowup_cc_complex_json(fc_stg):
    ...
    def fringe_cell_fc(c):
        if any(fc_stg.fc.rightfringe(s) for s in fc_stg.fc.star({c})):
            return True
        for b in fc_stg.fc.boundary({c}):
            if any(fc_stg.fc.rightfringe(s) for s in fc_stg.fc.star({b})):
                return True
        return False
...
def morse_sets_json(fc_stg, CMG, fibration):
    ...
    def non_fringe_top_cell(c):
        if fringe_cell_fc(c):
            return False
        # if any(fc_stg.fc.rightfringe(s) for s in fc_stg.fc.star({c})):
        # if fc_stg.fc.rightfringe(c):
        #    return False
        # Get cell dim from blowup cubical complex
        cell_dim = fc_stg.fc.cell_dim(c)
        if not (cell_dim == fc_stg.D):
            return False
        return True
...
def save_morse_graph_database_json(network, database_fname, param_indices=None,
    ...
    fc_stg = BlowupGraph(parameter, parameter_graph, 0)  # level = 0 for construction
    ...
    param_graph_json_data = parameter_graph_json(
        parameter_graph, param_indices, verts_colors, thres_type)
    ...
    for par_index in param_indices:
        ...
        parameter = parameter_graph.parameter(par_index)
        if fc_stg.D == 2:
            fc_stg = BlowupGraph(parameter, parameter_graph, level)
        else:
            fc_stg = BlowupGraph(parameter, parameter_graph, level)
