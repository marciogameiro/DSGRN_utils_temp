...
def PlotMorseSets(morse_graph, stg, graded_complex, cmap=None, clist=None, alpha=0.7,
    ...
    # Morse graph vertices indexing
    verts_index = {v: k for k, v in enumerate(sorted(morse_graph.vertices()))}
    # Collection of top cells to plot (collection of allowed top cells)
    allowed_top_cells = set([cell for cell in cell_complex(D) if allowed_cell(cell)])
